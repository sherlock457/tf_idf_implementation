...
def load_link_of_qs():
    with open("Leetcode-Questions-Scrapper/Qindex.txt","r",encoding='latin-1') as f:
        links=f.readlines()
    return links
...
def get_tf_dict(term):
    ...
    for doc in tf_dict:
        ...
        try:
            tf_dict[doc] /= len(documents[int(doc)])
...
def calc_docs_sorted_order(q_terms):
    ...
    for term in q_terms:
        ...
        for doc_index in potential_docs:
            #print("Question Link:", Qlink[int(doc_index)], "\tScore:", potential_docs[doc_index])
            ans.append({"Question Link": Qlink[int(doc_index)], "Score": potential_docs[doc_index]})
    return ans[:10:]
